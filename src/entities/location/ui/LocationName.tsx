...
interface LocationNameProps extends ComponentProps<'div'> {
  name: string;
  locationId: string;
}

export function LocationName(props: LocationNameProps) {
  const { name, locationId, className, ...rest } = props;
  return (
    <div
      className={`text-navy-600 max-sm:text-center ${className ?? ''}`}
      {...rest}
    >
      <Link to="/locations/$id" params={{ id: locationId }} className="cursor-pointer group">
