...
interface EpisodeNameProps extends ComponentProps<'div'> {
  name: string;
  episodeId: string;
}

export function EpisodeName(props: EpisodeNameProps) {
  const { name, episodeId, className, ...rest } = props;
  return (
    <div
      className={`text-navy-600 max-sm:text-center ${className ?? ''}`}
      {...rest}
    >
      <Link to="/episodes/$id" params={{ id: episodeId }} className="cursor-pointer group">
